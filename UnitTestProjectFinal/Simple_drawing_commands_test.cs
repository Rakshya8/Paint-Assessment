...
namespace UnitTestProjectFinal
{
    ...
    public class Simple_drawing_commands_test
    {
        ...
        public void check_executing_command()
        {
            ...
            {
                Assert.IsTrue(cv.Valid_execute_command(cmd));
            }
        }
        ...
        {
            //valid command with valid parameters
            Assert.IsTrue(cv.Check_command("drawto(20,30)"));
            //invalid parameter
            Assert.IsFalse(cv.Check_command("drawto(20,y)"));
            //invalid command name
            Assert.IsFalse(cv.Check_command("drwtos(20,30)"));
        }
        ...
        {
            //valid command with valid parameter
            Assert.IsTrue(cv.Check_command("polygon(20,40,50,50)"));
            //Invalid parameter length
            Assert.IsFalse(cv.Check_command("polygon(40,50)"));
            //invalid command name
            Assert.IsFalse(cv.Check_command("polgon(20,40,50)"));
        }
        ...
        {
            //valid command with valid parameter
            Assert.IsTrue(cv.Check_command("rotate(20)"));
            //Invalid parameter
            Assert.IsFalse(cv.Check_command("rotate(xy, 290)"));
            //Invalid command name
            Assert.IsFalse(cv.Check_command("rotae(120)"));
        }
